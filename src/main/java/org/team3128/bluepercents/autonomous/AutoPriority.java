...
public class AutoPriority extends CommandGroup{
    public AutoPriority(FalconDrive drive, Arm arm, BellIntake bellIntake, AHRS ahrs, Limelight limelight, DriveCommandRunning cmdRunning, double timeoutMs, ThreadScheduler scheduler){
        addSequential(new CmdDrive(drive, 1));
        addSequential(new CmdAutoTrajectory(drive, 130, 0.5, 10000, 
            new Pose2D(0, 0, Rotation2D.fromDegrees(0)),
            new Pose2D(0* Constants.MechanismConstants.inchesToMeters, -77.058419 * Constants.MechanismConstants.inchesToMeters, Rotation2D.fromDegrees(0))));//move to smartbell goal
